...
public class SGW implements IFloodlightModule, IOFMessageListener {
	...
	int contactPGW_Handover(IOFSwitch sgw, IOFSwitch pgw, DatapathId sgw_dpId, DatapathId pgw_dpId, String apn, String ue_ip, int sgw_teid_old){
		
		int sgw_teid, pgw_teid;
		...
		pgw_teid = SGW_PGW_TEID_MAP.get(sgw_teid_old);
		SGW_PGW_TEID_MAP.put(sgw_teid, pgw_teid);
		//uplink and downlink rules on PGW
		pg.installPGWRules(pgw, pgw_dpId, Constants.PGW_SGW_PORT_MAP.get(pgw_dpId.getLong() + Constants.SEPARATOR + sgw_dpId.getLong()), pgw_teid, sgw_teid, Constants.PGW_SINK_PORT, apn, ue_ip);
		
		//uplink rule (SGW to PGW)
		if(Constants.DEBUG){
			System.out.println("SGW installing uplink rule on S-GW dpid = "+sgw_dpId.getLong()+" inport="+Constants.SGW_PORT_MAP.get(sgw_dpId)[0]+" in teid = "+sgw_teid+
			" outPort = "+Constants.SGW_PORT_MAP.get(sgw_dpId)[1]+" out teid= "+pgw_teid+" of UE apn = "+apn);
		}
		
		installFlowRule(sgw, sgw_dpId, Constants.SGW_PORT_MAP.get(sgw_dpId)[0], sgw_teid, Constants.SGW_PORT_MAP.get(sgw_dpId)[1], pgw_teid, ue_ip, Constants.PGW_IP_UPLINK);
	 ...
	 */
	public boolean detachUEFromSGW(IOFSwitch sgw, IOFSwitch pgw, DatapathId sgw_dpId, DatapathId pgw_dpId, int sgw_teid, String ue_ip, Boolean isHandover){
		int pgw_teid;
