...
package net.floodlightcontroller.MTPdistr;
			...
			if(ipPkt.getProtocol().equals(IpProtocol.UDP)){
				...
				if(Data.class.isInstance(udpPkt.getPayload())){
					...
					if(payload.contains(Constants.SEPARATOR)){
						...
						case Constants.HANDOVER_REQUEST:
							//HANDOVER_REQUEST + SEPARATOR + longToString(apn) + SEPARATOR + longToString(key) + SEPARATOR + ue_ip + SEPARATOR + ue_teid + SEPARATOR + sgw_teid;
							if(Constants.DO_ENCRYPTION){
								decArray = receiveDecryptedArray(tmpArray);
							}
							...
							//uplink rule on SGW
							sgw_teid = sgw.contactPGW_Handover(switch_mapping.get(sgw_dpid),switch_mapping.get(pgw_dpid), sgw_dpid, pgw_dpid, tmpArray[1], tmpArray[3], Integer.parseInt(tmpArray[5])); //tmpArray[1] => apn of UE
							
							//install uplink rule on default switch
							if(Constants.DEBUG){
								System.out.println("DEFAULT SWITCH installing uplink rule on default switch dpid = "+defaultSwitch.getLong()+" inport="+uePort+" and SRC IP = "+tmpArray[3]+
										" outPort = "+Constants.ENODEB_SGW_PORT_MAP.get(Constants.DEFAULT_SWITCH_ID + Constants.SEPARATOR + sgw_dpid.getLong())+" and out SRC IP = "+Constants.RAN_IP+" out teid= "+sgw_teid+" of UE key = "+tmpArray[2]);
							}
							
							
							installFlowRuleWithIP(defaultSwitch, uePort, Constants.ENODEB_SGW_PORT_MAP.get(Constants.DEFAULT_SWITCH_ID + Constants.SEPARATOR + sgw_dpid.getLong()), sgw_teid, tmpArray[3], Constants.RAN_IP, Constants.SGW_IP_UPLINK, Constants.SINK_MAC);
							
							//MAP key = UE KEY,  MAP value = SGW_DPID + SEPARATOR + SGW_TEID(new)
							uekey_sgw_teid_map.put(tmpArray[2], sgw_dpid.toString() + Constants.SEPARATOR + sgw_teid);
							sgw_teid_uekey_map.put(Integer.toString(sgw_teid), tmpArray[2]); //////need to remove old one in perform handover.
							
							//downlink rule on SGW
							sgw.modifyBearerRequest(switch_mapping.get(sgw_dpid), sgw_dpid, sgw_teid, Integer.parseInt(tmpArray[4]), tmpArray[2]);


							if(Constants.DEBUG){
								//install downlink rule on default switch
								System.out.println("DEFAULT SWITCH installing downlink rule on default switch dpid = "+defaultSwitch.getLong()+" inport="+Constants.ENODEB_SGW_PORT_MAP.get(Constants.DEFAULT_SWITCH_ID + Constants.SEPARATOR + sgw_dpid.getLong())+
										" in teid = " + Integer.parseInt(tmpArray[4]) +
										" outPort = " + uePort + " out teid= " + Integer.parseInt(tmpArray[4]) + " of UE key = "+tmpArray[2]);
							}

							installFlowRule(defaultSwitch, Constants.ENODEB_SGW_PORT_MAP.get(Constants.DEFAULT_SWITCH_ID + Constants.SEPARATOR + sgw_dpid.getLong()), Integer.parseInt(tmpArray[4]), uePort, Integer.parseInt(tmpArray[4]), Constants.SINK_IP, tmpArray[3], Constants.UE_MAC);
							
							response = new StringBuilder();
							
							response.append(Constants.HANDOVER_ACCEPT).append(Constants.SEPARATOR).append(sgw_teid);
						...
						case Constants.PERFORM_HANDOVER:
							//PERFORM_HANDOVER + SEPARATOR + longToString(key) + SEPARATOR + ue_ip + SEPARATOR + ue_teid + SEPARATOR + sgw_teid(old)
							
							if(Constants.DEBUG){
								System.out.println("RECEIVED PERFORM HANDOVER from UE with ip=" + tmpArray[2] + " TEID=" + tmpArray[3] + " corresponding SGW TEID=" + tmpArray[4]);
							}
							
							tmp = uekey_sgw_teid_map.get(tmpArray[1]);
							tmpArray2 = tmp.split(Constants.SEPARATOR);  //sgw_dpid & sgw_teid -->new
							sgw_dpid = DatapathId.of(Constants.SGW_ID);
							pgw_dpid = DatapathId.of(Constants.PGW_ID);

							
							boolean status = sgw.detachUEFromSGW(switch_mapping.get(sgw_dpid), switch_mapping.get(pgw_dpid), sgw_dpid, pgw_dpid, Integer.parseInt(tmpArray[4]), tmpArray[2], true);
							
							response = new StringBuilder();
							response.append(Constants.HANDOVER_COMPLETE).append(Constants.SEPARATOR).append(tmpArray2[1]);
						...
						case Constants.DETACH_REQUEST:
							...
							status = sgw.detachUEFromSGW(switch_mapping.get(sgw_dpid), switch_mapping.get(pgw_dpid), sgw_dpid, pgw_dpid, Integer.parseInt(tmpArray[3]), tmpArray[1],false);
							response = new StringBuilder();
							if(status){
